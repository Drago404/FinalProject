			...
			$('.ui-slider-handle').on('mouseup', function()
			{
				$('.product_grid').isotope({
		            filter: function()
		            {
			        	...
			        	var priceMin = parseFloat(priceRange.split('-')[0].replace('лв.', ''));
			        	var priceMax = parseFloat(priceRange.split('-')[1].replace('лв.', ''));
			        	var itemPrice = $(this).find('.product_price').clone().children().remove().end().text().replace( 'лв.', '' );
